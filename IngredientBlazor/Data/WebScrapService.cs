    ...
    public class WebScrapervice
    {
        ...
        public async Task<ScrapedResult> ScrapUrlHttpAsync(string url)
        {
            ...
            try
            {
                ...
                //var temp = await httpClient.GetStreamAsync(url);
        ...
        {
            //with selnium might be best not to do task.when all i think, one by one might be better
            var scrapedResult = new ScrapedResult { Url = url, Status = ScrapedStatusEnum.InProgress };
            try
            {
                var content = string.Empty;
                var chromeOptions = new ChromeOptions();
                chromeOptions.AddArguments("--headless=new"); // Run Chrome in headless=new mode. Doesn't work in headless mode
                chromeOptions.AddArguments("--disable-gpu"); // Disable GPU acceleration
                chromeOptions.AddArguments("--no-sandbox"); // Disable sandbox mode
                ...
                //var browser_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.54 Safari/537.36 Edg/95.0.1020.30";
                //chromeOptions.AddArguments($"--user-agent={browser_agent}");
